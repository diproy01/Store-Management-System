...
<head>
    <meta charset="utf-8">
    <title>ISOLUTION - Add Purchase</title>
        ...
        $(function () {
            ...
            //document.getElementById('bill_no').focus();
			 $('#barcode').keydown(function (e) {
				if (e.which === 13) {
				var index = $('#barcode').index(this) + 1;
				//$('#barcode').eq(index).focus();
            ...
            $("#item").blur(function () {
				...
				document.getElementById('guid').value = document.getElementById('barcode').value
            });
			/*
           $("#item").blur(function () {
        ...
        function add_values() {
            if (unique_check()) {
                ...
                if (document.getElementById('edit_guid').value == "") {
                    if (document.getElementById('barcode').value != "" && document.getElementById('item').value != "" && document.getElementById('model').value != "" &&document.getElementById('quty').value != "" && document.getElementById('cost').value != "" && document.getElementById('total').value != "") {
                        ...
                        $('<tr id=' + item + '><td><input type=hidden value=' + item + ' id=' + item + 'id ><input type=text name="barcode[]" value=' + barcode + '  id=' + item + 'bcode style="width: 150px" class="round  my_with" ></td><td><input type=text name="stock_name[]"  id=' + item + 'st style="width: 150px" class="round  my_with" ></td><td><input type=text name=model[] value=' + model + ' id=' + item + 'm style="width: 80px" class="round  my_with" ></td><td><input type=text name=cost[] readonly="readonly" value=' + cost + ' id=' + item + 'c class="round  my_with" style="text-align:right;"></td><td><input type=text name=sell[] readonly="readonly" value=' + sell + ' id=' + item + 's class="round  my_with" style="text-align:right;"  ></td><td><input type=text name=quty[] readonly="readonly" value=' + quty + ' id=' + item + 'q class="round  my_with" style="text-align:right;" ></td><td><input type=text name=jibi[] readonly="readonly" value=' + total + ' id=' + item + 'to class="round  my_with" style="width: 120px;margin-left:20px;text-align:right;" ><input type=hidden name=total[] id=' + item + 'my_tot value=' + main_total + '> </td><td><input type=button value="" id=' + item + ' style="width:30px;border:none;height:30px;background:url(images/edit_new.png)" class="round" onclick="edit_stock_details(this.id)"  ></td><td><input type=button value="" id=' + item + ' style="width:30px;border:none;height:30px;background:url(images/close_new.png)" class="round" onclick= $(this).closest("tr").remove() ></td></tr>').fadeIn("slow").appendTo('#item_copy_final');

                       // $('<tr id=' + item + '><td><input type=hidden value=' + item + ' id=' + item + 'id ><input type=text name="stock_name[]"  id=' + item + 'st style="width: 150px" class="round  my_with" ></td><td><input type=text name="model[]" value=' + model + '   id=' + item + 'm style="width: 80px" class="round  my_with" ></td><td><input type=text name=cost[] readonly="readonly" value=' + cost + ' id=' + item + 'c class="round  my_with" style="text-align:right;"></td><td><input type=text name=sell[] readonly="readonly" value=' + sell + ' id=' + item + 's class="round  my_with" style="text-align:right;"  ></td><td><input type=text name=quty[] readonly="readonly" value=' + quty + ' id=' + item + 'q class="round  my_with" style="text-align:right;" ></td><td><input type=text name=jibi[] readonly="readonly" value=' + total + ' id=' + item + 'to class="round  my_with" style="width: 120px;margin-left:20px;text-align:right;" ><input type=hidden name=total[] id=' + item + 'my_tot value=' + main_total + '> </td><td><input type=button value="" id=' + item + ' style="width:30px;border:none;height:30px;background:url(images/edit_new.png)" class="round" onclick="edit_stock_details(this.id)"  ></td><td><input type=button value="" id=' + item + ' style="width:30px;border:none;height:30px;background:url(images/close_new.png)" class="round" onclick= $(this).closest("tr").remove() ></td></tr>').fadeIn("slow").appendTo('#item_copy_final');
    ...
    <div class="page-full-width cf">
        ...
        <div class="side-content fr">
			...
			$barcode = "";
				...
				$catagory=$db->queryUniqueValue("select category from stock_avail where name='$stock_name[$i]'");
                                $count = $db->countOf("stock_details", "stock_name='$stock_name[$i]' and stock_brand='$model[$i]' and barcode='$barcode[$i]'");
                                if ($count == 0) {
				...
				$amount = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE name='$stock_name[$i]'");
                                    ...
                                    $db->query("insert into stock_details(barcode,category,stock_id,stock_name,stock_brand,stock_quatity,supplier_id,company_price,selling_price) values('$barcode[$i]','$catagory','$autoid','$stock_name[$i]','$model[$i]','$quty[$i]','$supplier','$cost[$i]','$sell[$i]')");


                                    $db->query("INSERT INTO stock_entries(barcode,category,stock_id,stock_name,stock_brand, stock_supplier_name, quantity, company_price, selling_price, opening_stock, closing_stock, date, username, type, total, payment, balance, mode, description, due, subtotal,count1,billnumber) VALUES ( '$barcode[$i]','$catagory','$autoid1','$stock_name[$i]','$model[$i]','$supplier','$quty[$i]','$cost[$i]','$sell[$i]',0,'$quty[$i]','$date','$username','entry','$total[$i]','$payment','$balance','$mode','$description','$due','$subtotal',$i+1,'$bill_no')");
                                    ...
                                    $amount1 = $amount + $quty[$i];
                                    $db->execute("UPDATE stock_avail SET quantity='$amount1' WHERE name='$stock_name[$i]'");
									$amount22 = $db->queryUniqueValue("SELECT stock_quatity FROM stock_details WHERE stock_name='$stock_name[$i]' and stock_brand='$model[$i]' and barcode='$barcode[$i]'");
                                    $amount221 = $amount22 + $quty[$i];
									$db->execute("UPDATE stock_details SET stock_quatity='$amount221' WHERE stock_name='$stock_name[$i]' and stock_brand='$model[$i]' and barcode='$barcode[$i]'");
                                    $db->query("INSERT INTO stock_entries(barcode,category,stock_id,stock_name,stock_brand,stock_supplier_name,quantity,company_price,selling_price,opening_stock,closing_stock,date,username,type,total,payment,balance,mode,description,due,subtotal,count1,billnumber) VALUES ('$barcode[$i]','$catagory','$autoid1','$stock_name[$i]','$model[$i]','$supplier','$quty[$i]','$cost[$i]','$sell[$i]','$amount','$amount1','$date','$username','entry','$total[$i]','$payment','$balance','$mode','$description','$due','$subtotal',$i+1,'$bill_no')");
                    ...
                    <form name="form1" method="post" id="form1" action="">
                        ...
                        <table class="form">
                            ...
                            <tr> 
							<td><input name="" type="text" id="barcode" maxlength="200"
                                           class="round default-width-input" style="width: 100px"/></td>
                                <td>  
								<input name="" type="text" id="item" maxlength="200"
