                ...
                if (document.getElementById('edit_guid').value == "") {
                    if (document.getElementById('item').value != "" && document.getElementById('model').value != "" && document.getElementById('quty').value != "" && document.getElementById('total').value != "") {
                        ...
                        if (document.getElementById('quty').value != 0) {
                            ...
                            $('<tr id=' + item + '><td><input type=hidden value=' + item + ' id=' + item + 'id ><input type=hidden name="barcode[]" value=' + barcode + ' id=' + item + 'bcode style="width: 80px" class="round  my_with" ></td><td><input type=text name="stock_name[]"  id=' + item + 'st value=' + code + ' style="width: 150px" class="round  my_with" ></td><td><input type=text name="model[]" value=' + model + ' id=' + item + 'm style="width: 80px" class="round  my_with" ></td><td><input type=text name=sell[] readonly="readonly" value=' + sell + ' id=' + item + 's class="round  my_with" style="text-align:right;"  ></td><td><input type=text name=quty[] readonly="readonly" value=' + quty + ' id=' + item + 'q class="round  my_with" style="text-align:right;" ></td><td><input type=text name=stock[] readonly="readonly" value=' + disc + ' id=' + item + 'p class="round  my_with" style="text-align:right;" ></td><td><input type=text name=jibi[] readonly="readonly" value=' + total + ' id=' + item + 'to class="round  my_with" style="width: 120px;margin-left:20px;text-align:right;" ><input type=hidden name=total[] id=' + item + 'my_tot value=' + main_total + '> </td><td><input type=button value="" id=' + item + ' style="width:30px;border:none;height:30px;background:url(images/edit_new.png)" class="round" onclick="edit_stock_details(this.id)"  ></td><td><input type=button value="" id=' + item + ' style="width:30px;border:none;height:30px;background:url(images/close_new.png)" class="round" onclick=reduce_balance("' + item + '");$(this).closest("tr").remove(); ></td></tr>').fadeIn("slow").appendTo('#item_copy_final');
            ...
            <div class="content-module">
				...
				$model = $_POST['model'][$i];
                                ...
                                $catagory=$db->queryUniqueValue("select category from stock_avail where name='$name1'");
                                  $company_price=intval($db->queryUniqueValue("select company_price from stock_details where stock_name='$name1' and stock_brand='$model'"));
                                   $profit=($rate-$company_price)*$quantity;
                                    
                                if ($count >= 1) {
                                    $db->query("insert into stock_sales (tax,tax_dis,discount,dis_amount,grand_total,transactionid,stock_name,selling_price,quantity,amount,date,username,customer_id,subtotal,payment,balance,due,mode,description,count1,billnumber,category,stock_brand,profit)
                            values('$tax','$tax_dis','$discount','$dis_amount','$payable','$autoid','$name1','$rate','$quantity','$total','$mysqldate','$username','$customer','$subtotal','$payment','$balance','$due','$mode','$description',$i+1,'$bill_no','$catagory','$model','$profit')");

                                    $amount = $db->queryUniqueValue("SELECT quantity FROM stock_avail WHERE name='$name1'");
                                    $amount1 = $amount - $quantity;

                                    $db->query("insert into stock_entries (stock_id,stock_name,quantity,opening_stock,closing_stock,date,username,type,salesid,total,selling_price,count1,billnumber,stock_brand) values('$autoid','$name1','$quantity','$amount','$amount1','$mysqldate','$username','sales','$autoid','$total','$rate',$i+1,'$bill_no','$model')");
                                    //echo "<br><font color=green size=+1 >New Sales Added ! Transaction ID [ $autoid ]</font>" ;


                                    $amount33 = $db->queryUniqueValue("SELECT stock_quatity FROM stock_details WHERE stock_name='$name1' and stock_brand='$model'");
                    ...
                    <form name="form1" method="post" id="form1" action="">
                     ...
                     <!-- <input type="button" style="height:30px; width:50px;" id="btn" value=" ADD ">-->
						...
						<input type="hidden" id="guid2">
                        <input type="hidden" id="edit_guid2">
						<br/><br/>
                        <table class="form">
                            <tr>
                                <td>Item:</td>
